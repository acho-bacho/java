...
public class AllPalindromes {
    ...
    private static void allPalPartitions(String input) {
        ...
        Deque<String> currPartition = new LinkedList<String>();

        // Call recursive function to generate all partiions and store in allPart
        allPalPartitonsUtil(allPartitions, currPartition, 0, n, input);
 ...
 /*
   ...
   */
    ...
    private static void allPalPartitonsUtil(ArrayList<ArrayList<String>> allPartitions, Deque<String> currIterPartition, int start, int end, String input) {
        // If 'start' has reached len
        if (start >= end) {
            allPartitions.add(new ArrayList<>(currIterPartition));
            return;
        }
        ...
        for (int i = start; i < end; i++) {
            ...
            if (isPalindrome(input, start, i)) {
                ...
                currIterPartition.addLast(input.substring(start, i + 1));

                // Recur for remaining substring
                allPalPartitonsUtil(allPartitions, currIterPartition, i + 1, end, input);

                // Remove substring str[start..i] from current partition
                currIterPartition.removeLast();
            }
        }
    }
